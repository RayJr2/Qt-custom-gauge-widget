...
void QcLabelItem::draw(QPainter *painter)
{
    ...
    QFont font("Meiryo UI", r/mFontSize, QFont::Bold);
...
void QcLabelItem::setVisible(bool visible)
{
    mVisible = visible;
}

bool QcLabelItem::isVisible()
{
    return mVisible;
}

void QcLabelItem::setFontSize(int fontSize)
{
    // Only set font size if new size is reasonable
...
{
    //Ray fix for commpass being 90 deg off
    if (mNeedleType == NeedleType::CompassNeedle) {
        int compassValue = value + 90;
        if (compassValue > 360)
            compassValue = compassValue - 360;
        mCurrentValue = compassValue;
    } else {
        if (value < mMinValue)
...
*/
    ...
    if (mLabel != 0)
        //Ray added support for precision and display text value outside meter scale
        mLabel->setText(QString::number(value, 'f', mPercision),false);
...
void QcNeedleItem::setPercision(int percision)
{
    if (percision >= 0 && percision <= 6) {
        mPercision = percision;
    }
}

void QcNeedleItem::setVisible(bool visible)
{
    mVisible = visible;
}

bool QcNeedleItem::isVisible()
{
    return mVisible;
}
